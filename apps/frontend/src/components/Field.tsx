...
export function FieldWrapper({
  ...
  return (
    <label className={`block ${className}`}>
      <span
        className={`mb-1.5 block text-sm font-medium text-slate-900 ${labelClassName}`}
      >
        {label}
      </span>
      {children}
...
const INPUT_BASE =
  "block w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 focus:border-slate-500 focus:outline-none focus:ring-2 focus:ring-slate-200 disabled:bg-slate-100 disabled:text-slate-500";
